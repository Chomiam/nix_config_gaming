...
let
  ...
  setupScript = pkgs.writeShellScriptBin "chomiamos-desktop-setup" ''
    ...
    if [ "$FORCE" -eq 1 ] || [ ! -f "$SENTINEL" ]; then
      ...
      ${pkgs.coreutils}/bin/ln -sf "${gtk4LibadwaitaCss}" "$HOME/.config/gtk-4.0/gtk.css"
      ${pkgs.coreutils}/bin/ln -sf "${gtk4LibadwaitaCss}" "$HOME/.config/gtk-4.0/gtk-dark.css"
      if [ -L "$HOME/.config/gtk-4.0/assets" ] || [ -d "$HOME/.config/gtk-4.0/assets" ]; then
        ${pkgs.coreutils}/bin/rm -rf "$HOME/.config/gtk-4.0/assets"
      fi
...
in
{
  ...
  environment.etc."xdg/gtk-4.0/gtk.css".source = gtk4LibadwaitaCss;
  environment.etc."xdg/gtk-4.0/gtk-dark.css".source = gtk4LibadwaitaCss;
