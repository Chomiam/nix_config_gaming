...
let
  ...
  ollamaPackage =
    if cfg.ollama.acceleration == "rocm" then
      pkgs.ollama-rocm
    else if cfg.ollama.acceleration == "cuda" then
      pkgs.ollama-cuda
    else if cfg.ollama.acceleration == "vulkan" then
      pkgs.ollama-vulkan
    else if cfg.ollama.acceleration == "cpu" then
      pkgs.ollama-cpu
    else
  ...
  rocmGfxOverride =
    if (gpu == "amd" || cfg.ollama.acceleration == "rocm") then cfg.ollama.rocmOverrideGfx else null;
in
{
  # =========================================================================
  ...
  config = lib.mkIf cfg.enable {
    ...
    # 1. 🦭 Activation automatique du runtime Podman si Open WebUI est activé (Hermes est désormais natif)
    ...
    hardware.amdgpu.opencl.enable = lib.mkIf (
      cfg.ollama.enable && (gpu == "amd" || cfg.ollama.acceleration == "rocm")
    ) true;

    # Avertissement si le GPU est Nvidia Legacy (pilote 470 incompatible avec CUDA 12)
    warnings =
      lib.optional (cfg.ollama.enable && gpu == "nvidia-legacy" && cfg.ollama.acceleration == "auto")
    ...
    virtualisation.oci-containers.containers.open-webui = lib.mkIf cfg.openWebUI.enable {
      ...
      environment = {
        ...
        OPENAI_API_BASE_URLS =
          if cfg.hermes.enable then "http://127.0.0.1:${toString cfg.hermes.apiPort}/v1" else "";
    ...
    systemd.services.hermes-agent = lib.mkIf cfg.hermes.enable {
      ...
      after = [ "network-online.target" ] ++ lib.optional cfg.ollama.enable "ollama.service";
      wants = [ "network-online.target" ] ++ lib.optional cfg.ollama.enable "ollama.service";
